...
const plans = [
  ...
  { name: 'Pro', price: '$29', period: '/mo', tokens: 350, description: '350 tokens/month', features: ['350 AI generations', 'All styles', '4K export', 'No watermark', 'Priority support'], cta: 'Get Pro', href: null, highlighted: true, priceId: 'price_pro' },
  { name: 'Studio', price: '$59', period: '/mo', tokens: 1000, description: '1000 tokens/month', features: ['1000 AI generations', 'Custom watermark', '4K export', 'API access', 'Dedicated support'], cta: 'Get Studio', href: null, highlighted: false, priceId: 'price_studio' },
]
...
export default function PricingSection() {
  ...
  const handleCheckout = async (priceId: string) => {
    ...
    try {
      ...
      else alert('Add STRIPE_SECRET_KEY to Vercel Environment Variables to enable payments.')
    ...
    <section id="pricing" style={{ background: '#0a0a0f', padding: '6rem 0', borderTop: '1px solid #2a2a3a' }}>
      <div style={{ maxWidth: 1280, margin: '0 auto', padding: '0 2rem' }}>
        <div style={{ textAlign: 'center', marginBottom: '3rem' }}>
          ...
          <p style={{ color: '#9999aa' }}>1 generation = 1 token. No hidden fees.</p>
        </div>
          ...
          {plans.map(plan => (
            <div key={plan.name} className="glass-card" style={{ padding: '2rem', borderRadius: 20, border: plan.highlighted ? '1px solid rgba(201,168,76,0.4)' : '1px solid #2a2a3a', boxShadow: plan.highlighted ? '0 0 40px rgba(201,168,76,0.1)' : 'none', position: 'relative', transition: 'transform 0.2s' }}
              ...
              <div style={{ marginBottom: '1.5rem' }}>
                <div style={{ fontWeight: 700, color: '#f5f5f0', marginBottom: '0.5rem', fontSize: '1.1rem' }}>{plan.name}</div>
                ...
                <div style={{ fontSize: '0.8rem', color: '#9999aa', marginBottom: '0.5rem' }}>{plan.description}</div>
                <div style={{ display: 'inline-block', background: 'rgba(201,168,76,0.1)', border: '1px solid rgba(201,168,76,0.2)', borderRadius: '9999px', padding: '0.2rem 0.6rem', fontSize: '0.75rem', color: '#c9a84c' }}>
