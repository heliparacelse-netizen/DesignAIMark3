          ...
          {mockHistory.map(item => (
            <div key={item.id} className="glass-card" style={{ padding: '1.25rem', borderRadius: 16, display: 'flex', alignItems: 'center', gap: '1.5rem', transition: 'transform 0.2s' }}
              ...
              <div style={{ display: 'flex', gap: '0.5rem' }}>
                ...
                <Link href="/studio">
                  <button style={{ padding: '0.5rem 1rem', borderRadius: 8, background: '#2a2a3a', border: 'none', color: '#9999aa', cursor: 'pointer', fontSize: '0.8rem', display: 'flex', alignItems: 'center', gap: '0.35rem' }}><RotateCcw size={14} />Reuse</button>
                </Link>
              </div>
            </div>
          ))}
